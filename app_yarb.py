...
Fix: system_prompt now contains all required smolagents placeholders.
Fix v2: UTF-8 encoding fix for Windows charmap codec errors with emoji characters.
"""
...
# Force UTF-8 for stdout/stderr — prevents charmap codec errors on Windows
...
#  Strips or replaces emoji/special Unicode that can crash charmap
# ─────────────────────────────────────────────────────────────────

def safe_str(text: str) -> str:
    """
    Encode the string to the current stdout encoding, replacing any
    characters that cannot be represented (e.g. emoji on Windows charmap).
    Falls back to ASCII with xmlcharrefreplace so nothing is lost silently.
    """
    try:
...
def build_data_context(df: pd.DataFrame) -> str:
    """Pre-compute all real statistics — injected into LLM calls to prevent hallucination."""
    rows, ncols  = df.shape
...
def clean_output(text: str) -> str:
    """Strip any leaked agent internals from the final output."""
    text = str(text).strip()
    # Remove code fences
    text = re.sub(r"```[\s\S]*?```", "", text)
    # Remove header-style internal sections
    text = re.sub(
        r"(?im)^[ \t]*(thoughts?|code|observation|action\s*input|action)\s*:.*?(?=\n\n|\Z)",
        "", text, flags=re.DOTALL
    )
    # Remove lines that look like Python code
    py_pat = re.compile(
...
#  DataFrames live in st.session_state (smolagents can only pass primitives)
#
#  NOTE: All tool return strings use only ASCII-safe characters to avoid
#  charmap codec errors on Windows. Emoji are replaced with text labels.
# ─────────────────────────────────────────────────────────────────
...
def summarise_dataset() -> str:
    ...
    # Plain-English output — ASCII-safe (no emoji that crash charmap)
    out = [
...
def analyse_time_series(
    ...
    # Validate frequency and aggregation
    ...
    if len(series) < 2:
        return ("Not enough time periods to draw a trend. "
    ...
    # Chart — use ASCII-safe text in titles/labels
    ...
    ax.annotate(
        f"Peak\n{fmt(series.max())}",
        xy=(pk_d, series.max()), xytext=(10, 14), textcoords="offset points",
        fontsize=8, color=P[0], arrowprops=dict(arrowstyle="->", color=P[0], lw=1.2)
    )
...
def run_classification(
    ...
    return (
        ...
        + f"\nModel used: {model_name}. Charts above show the confusion matrix, F1 per class, and feature importance."
    )
...
def answer_data_question(question: str) -> str:
    ...
    Do NOT call this for full summary requests (use summarise_dataset),
    time series requests (use analyse_time_series), or classification requests (use run_classification).
        ...
        for col in num:
            if col.lower() in q:
                return (f"**`{col}`** -- mean: **{fmt(df[col].mean())}**, "
                        f"median: {fmt(df[col].median())}, "
                        f"std: {fmt(df[col].std())}")
    ...
    # Fallback: return the pre-computed context so the LLM can reason from it
    return (
...
# ─────────────────────────────────────────────────────────────────
#  AGENT — system_prompt MUST contain all three smolagents placeholders
# ─────────────────────────────────────────────────────────────────
...
6. If no date column exists and the user asks about time trends, explain this clearly using the tool result.
...
def run_agent(prompt: str, model_id: str, api_key: str) -> str:
    """Run the agent with retry on rate-limit errors."""
    import litellm
    max_retries = 2
    for attempt in range(max_retries + 1):
        try:
            ...
            err = str(e)
            # Extra guard: if a charmap error somehow still leaks through, report it clearly
            if "charmap" in err.lower() or "codec" in err.lower():
                return (
                    "WARNING: A text encoding error occurred. This usually means a special "
                    "character could not be processed. Please try again -- the issue is transient."
                )
...
with st.sidebar:
    ...
    if df is not None:
        ...
        ts_agg    = st.selectbox("Aggregation",
                                  ["mean -- Average","sum -- Total","median -- Median","max -- Max","min -- Min"],
                                  key="ts_agg")
...
<div class="app-header">
  ...
  <div>
    ...
    <p>Upload a CSV &rarr; chat to summarise, find trends, classify, or ask any question about your data.</p>
  </div>
...
if st.session_state["data"] is None:
    ...
    <em>"classify the target"</em> &middot; <em>"how many missing values?"</em>
    </p>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
# ── Determine final prompt (typed or chip) ────────────────────────────────────
final_prompt = None
