"""
Smart Data Analyst — v5 (Chat-First, Fixed)
Full chat interface: type or click chips -> agent picks the right @tool automatically.
Fix v1: system_prompt contains all required smolagents placeholders.
Fix v2: UTF-8 encoding fix for Windows charmap codec errors.
Fix v3: Partial-period drop, better trend window, bar chart layout.
"""

# ── UTF-8 ENCODING FIX (must be first) ────────────────────────────────────────
import os
import sys
...
def analyse_time_series(
    ...
    if not time_column or time_column not in df.columns:
        ...
        return (
            ...
            "(named 'date', 'time', 'timestamp', 'year', etc.).\n\n"
    ...
    if target_column not in df.columns:
        return f"Column `{target_column}` not found. Available: {', '.join(df.columns.tolist())}"
    if not pd.api.types.is_numeric_dtype(df[target_column]):
        return f"`{target_column}` is not numeric. Please specify a numeric column."
    ...
    # ─────────────────────────────────────────────────────────────
    # FIX 1: DROP INCOMPLETE LAST PERIOD
    # ─────────────────────────────────────────────────────────────
    # Problem: When resampling to M/W/Q with sum or count, the final period
    # (e.g. February 2025 if today is Feb 24) contains fewer source rows than
    # a full period, so its aggregate is artificially low. This creates a fake
    # cliff-shaped drop at the right end of the chart.
    #
    # Detection: count raw rows per period; if the last period's row count is
    # less than 70% of the median row count of all other periods, it is
    # incomplete and we drop it before charting and computing statistics.
    # ─────────────────────────────────────────────────────────────
    dropped_last  = False
    dropped_label = ""
    if len(series) >= 3:
        raw_counts   = df2[target_column].resample(frequency).count()
        median_count = raw_counts.iloc[:-1].median()
        last_count   = raw_counts.iloc[-1]
        if median_count > 0 and (last_count / median_count) < 0.70:
            dropped_label = series.index[-1].strftime("%Y-%m-%d")
            series        = series.iloc[:-1]   # drop the incomplete period
            dropped_last  = True

    if len(series) < 2:
        return ("Not enough complete time periods to draw a trend. "
                "Try frequency='D' (daily) or 'W' (weekly).")

    # ─────────────────────────────────────────────────────────────
    # FIX 2: BETTER ROLLING WINDOW
    # ─────────────────────────────────────────────────────────────
    # Old: max(2, len//6)  ->  gives window=2 for 14 months (too noisy)
    # New: ~25% of length, clamped between 3 and 6 -> smoother, more useful
    # ─────────────────────────────────────────────────────────────
    roll_w = max(3, min(6, len(series) // 4))
    ...
    pk_d  = series.idxmax()
    lw_d  = series.idxmin()
    pct   = ((series.iloc[-1] - series.iloc[0]) / abs(series.iloc[0]) * 100
    ...
    AL = {"mean":"Average","sum":"Sum","median":"Median","max":"Max","min":"Min"}

    # ─────────────────────────────────────────────────────────────
    # FIX 3: CHART — taller figure + explicit hspace so bar panel shows
    # ─────────────────────────────────────────────────────────────
    fig, axes = plt.subplots(
        2, 1, figsize=(14, 9), facecolor="#F2F4F1",
        gridspec_kw={"height_ratios": [3, 1.2], "hspace": 0.38}
    )
    trend_tag = "(up)" if trend == "upward" else "(down)"
    fig.suptitle(
        f"{target_column} Over Time {trend_tag}  |  "
    ...
    ax.plot(series.index, series.values,
            lw=2, color=P[0], marker="o", markersize=5, label=target_column)
    ax.plot(roll.index, roll.values,
            lw=2.5, color=WARN, linestyle="--",
            label=f"Smoothed trend (window={roll_w})")
    ax.axhline(avg, color=P[3], linestyle=":", lw=1.8, label=f"Avg: {fmt(avg)}")
    ax.annotate(
        f"Peak\n{fmt(series.max())}",
        xy=(pk_d, series.max()), xytext=(12, 14), textcoords="offset points",
    ...
    try:
        ...
        ax2.bar(series.index, series.values,
                width=bw, color=bar_c, alpha=.85, edgecolor="white", lw=.5)
    except Exception:
        ax2.bar(range(len(series)), series.values,
                color=bar_c, alpha=.85, edgecolor="white", lw=.5)
    ax2.axhline(avg, color=P[3], linestyle=":", lw=1.5, label=f"Avg: {fmt(avg)}")
    chart_style(ax2, xlabel="Date", ylabel=AL.get(aggregation, aggregation))
    ax2.legend(fontsize=8, framealpha=.7)

    # Footnote when an incomplete period was excluded
    if dropped_last:
        fig.text(
            0.99, 0.005,
            f"Note: last period ({dropped_label}) excluded -- incomplete data",
            ha="right", va="bottom", fontsize=8, color=MID, style="italic"
        )
    ...
    note = (
        f"\n- NOTE: The last period ({dropped_label}) was excluded because it "
        f"appears incomplete (fewer records than a typical {FL.get(frequency,'').lower()} period). "
        f"The trend above reflects only complete periods."
        if dropped_last else ""
    )

    return (
        f"[CHART] Time Series: `{target_column}` -- "
        f"{AL.get(aggregation, aggregation)} by {FL.get(frequency, frequency)}\n\n"
        f"- Overall trend: **{trend}** ({pct:+.1f}% from first to last complete period)\n"
        ...
        f"- Complete periods analysed: **{len(series)}** {FL.get(frequency,'').lower()} periods"
        f"{note}\n\n"
        f"Smoothed trend uses a rolling window of {roll_w} periods for a cleaner signal."
    )
...
def run_classification(
    ...
    Call this when the user asks: classify, predict, train a model, ML analysis,
    which features matter, what predicts X, classification, machine learning.
    ...
    return (
        ...
        + (f"- WARNING: Weak classes: {', '.join(str(c) for c in weak)} (F1<0.5 -- collect more examples)\n"
           if weak else "- All classes scored F1 >= 0.5 -- model is well balanced\n")
        + f"\nModel used: {model_name}. Charts show confusion matrix, F1 per class, and feature importance."
    )
...
def answer_data_question(question: str) -> str:
    ...
    Do NOT call this for full summary (use summarise_dataset), time series (use analyse_time_series),
    or classification (use run_classification).
    ...
    if any(w in q for w in ["missing","null","nan","empty","incomplete","blank"]):
        ...
        return (f"**Missing values in {len(miss)} out of {ncols} columns "
                f"(total {miss.sum():,} cells):**\n" + "\n".join(items))
    if "duplicate" in q:
        dups = int(df.duplicated().sum())
        return ("No duplicate rows found." if dups == 0
                else f"WARNING: {dups:,} duplicate rows ({dups/rows*100:.1f}% of data).")
    ...
    return (
        f"Here are the verified statistics for this dataset:\n\n{ctx}\n\n"
...
SYSTEM_PROMPT = """You are a friendly, expert Data Analyst assistant in a chat interface.
...
HOW TO CHOOSE A TOOL -- pick exactly one per user message:
- summarise_dataset()         -> user asks for overview, summary, describe, explore, profile the data
- analyse_time_series(...)    -> user asks about trends, time patterns, monthly/weekly, how X changed over time
- run_classification(...)     -> user asks to classify, predict, train a model, run ML, what drives X
- answer_data_question(...)   -> any other factual question: rows, columns, missing values, averages, correlations, max/min

RULES:
1. ALWAYS call one tool -- never answer from your own training knowledge.
2. Use ONLY the numbers returned by the tool. Never invent or estimate statistics.
3. Write a clear, concise plain-English response using bullet points and **bold** for key numbers.
4. Never output Python code, code blocks, or internal sections like Thoughts:/Code:/Observation:.
5. End with one practical suggestion when appropriate.
6. If the tool result includes a NOTE about an excluded period, always mention it clearly to the user.
...
def make_agent(model_id: str, api_key: str) -> ToolCallingAgent:
    ...
    return ToolCallingAgent(
        tools=[summarise_dataset, analyse_time_series, run_classification, answer_data_question],
        model=llm,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                return (f"WARNING: Rate limit reached. Wait ~{ws}s and try again, "
                        "or switch to a different model in the sidebar.")
        except Exception as e:
            err = str(e)
            if "charmap" in err.lower() or "codec" in err.lower():
                return "WARNING: Encoding error. Please try again."
...
with st.sidebar:
    ...
    if df is not None:
        ...
        ts_freq   = st.selectbox("Frequency",
                                  ["M -- Monthly","D -- Daily","W -- Weekly","Q -- Quarterly","Y -- Yearly"],
                                  key="ts_freq")
        ts_agg    = st.selectbox("Aggregation",
                                  ["sum -- Sum","mean -- Average","median -- Median","max -- Max","min -- Min"],
                                  key="ts_agg")
        ...
        model_opts = {
            ...
            "Llama 3.1 70B -- Balanced": "groq/llama-3.1-70b-versatile",
        }
        ...
        ts_col_v = ts_val_v = ts_freq_v = "auto"
        ts_agg_v = "sum"
...
<div class="app-header">
  ...
  <div>
    ...
    <p>Upload a CSV &rarr; chat to summarise, find trends, classify, or ask anything about your data.</p>
  </div>
...
if not groq_key:
    ...
    Create a <code>.env</code> file and add: <code>GROQ_API_KEY=your_key_here</code><br>
...
chip_defs = {
    ...
    "Classify":       f"Run classification to predict '{clf_tgt}' using features: {', '.join(sel_feats) if sel_feats else 'auto'}, model: {clf_model}.",
    ...
    "Averages":       "What is the average value of each numeric column?",
}
...
for msg in st.session_state["messages"]:
    if msg["role"] == "user":
        ...
        st.markdown(
            f'<div class="row-bot">'
            f'<div class="avatar avatar-bot">A</div>'
            f'<div class="bubble-bot">{content_html}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
...
with st.form("chat_form", clear_on_submit=True):
    ...
    user_text = col_in.text_input(
        "msg",
        placeholder="Type anything... e.g. 'how many rows?', 'analyse trends', 'classify species'",
